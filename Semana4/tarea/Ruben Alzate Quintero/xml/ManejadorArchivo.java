...
public class ManejadorArchivo {
	...
	private static final String RUTA_ARCHIVO = "c:\\users\\RUBEN\\documents\\base_datos.xml";
	...
	public void eliminarAsegurado(String busqueda) 
		...
		System.out.println("Resultado de la búsqueda: ");
		int borrar;
		NodeList nodes = document.getElementsByTagName("Asegurado"); //Listo los nodos de Asegurado
		for (int i = 0; i < asegurados.getLength(); i++) {
			Node nombre = (Node) xpath.evaluate("Nombre", asegurados.item(i), XPathConstants.NODE);
			System.out.println("Nombre: " + nombre.getTextContent());
			...
			Node direccion = (Node) xpath.evaluate("Direccion", asegurados.item(i), XPathConstants.NODE);
			System.out.println("Dirección: " + direccion.getTextContent());
			System.out.print("\n\nEste es el asegurado que desea eliminar? 1.Si  2.No"); //Imprimo si quiere borrar
			borrar = new Scanner(System.in).nextInt();
			
			Element nombreAsegurado = (Element)asegurados.item(i); //Le asigno la lista de asegurados a nombreAsegurado
			if(borrar==1){		       
		        // Elimino el nodo actual con todos sus hijos, importante, el control esta en i
		        nombreAsegurado.getParentNode().removeChild(nombreAsegurado);
			}
		}
		// Grabo los datos actuales de "document"
        TransformerFactory transformerFactory = TransformerFactory.newInstance();
		Transformer transformer = transformerFactory.newTransformer();
